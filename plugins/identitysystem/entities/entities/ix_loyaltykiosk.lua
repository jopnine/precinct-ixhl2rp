...
if (SERVER) then
	function ENT:SpawnFunction(client, trace)
		local dispenser = ents.Create("ix_loyaltykiosk")
	...
	function ENT:SpawnRation(callback, releaseDelay, item)
		releaseDelay = releaseDelay or 1.2

		local item = ix.item.Spawn(item, self:GetPos(), function(itemTable, entity)
			if (callback) then
				callback(entity)
			end
	...
	function ENT:StartDispense(item)
		self:SetDisplay(3)
	...
	function ENT:Draw()
		...
		if self:GetNetVar("destroyed", false) == false then
			draw.SimpleText(display[1], "ixRationDispenser", 86, 36, color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		else
			draw.SimpleText("ERRRRR...", "ixRationDispenser", 86, 36, Color(255,0,0,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
...
function ENT:OnTakeDamage(dmginfo)
	if not self:GetNetVar("health", 100) then
		self:GetNetVar("health", 100)
	else
		self:SetNetVar("health", self:GetNetVar("health", 100) - dmginfo:GetDamage())

		if self:GetNetVar("health", 100) <= 0 then
			self:DestroyLogic()
		end
	end
end

function ENT:DestroyLogic()
	if not self:GetNetVar("destroyed", false) then
		local ent = ents.Create("npc_grenade_frag")
		ent:SetPos(self:GetPos())
		ent:Spawn()
		ent:Fire("SetTimer", 0.1)
