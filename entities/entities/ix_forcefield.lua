...
if (SERVER) then
	...
	local MODES = {
		...
		},
		{
			...
			"Apenas permitir com CID válido."
		},
		{
			function(client)
				return true
		...
		function(client)
			...
			if (character and character:GetInventory() and !character:GetInventory():HasItem("stc")) then
				return true
			else
				return false
			end
		end,
		"Apenas permitir membros do sindicato com CID válido."
		},
		{
			function(client)
				local character = client:GetCharacter()
			...
			"Apenas permitir membros do departamento de pesquisa com CID válido."
		},
		{
			function(client)
				local character = client:GetCharacter()
			...
			"Apenas permitir membros do departamento médico com CID válido."
		}
	}
	...
	function ENT:Use(activator)
		...
		if (activator:IsCombine()) then
			...
			activator:ChatPrint("Alterando modo da barreira para: "..MODES[self:GetMode()][2])
